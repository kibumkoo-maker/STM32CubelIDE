...
class HC06Bluetooth:
    ...
    # 명령어 형식 (펌웨어 버전별)
    COMMAND_FORMATS = {
        'type1': {  # AT 명령어 타입1 (AT+UART, AT+PSWD:"xxxx")
            'test': ['AT'],
            'version': ['AT+VERSION'],
            'name': ['AT+NAME{}'],
            'pin': ['AT+PSWD:"{}"', 'AT+PSWD{}'],
        ...
        'type2': {  # AT 명령어 타입2 (AT+BAUDx, AT+NAMExxxx, AT+PINxxxx)
            'test': ['AT'],
            'version': ['AT+VERSION'],
            'name': ['AT+NAME{}'],
            'pin': ['AT+PIN{}'],
        ...
        'type3': {  # 혼합형 (다양한 변형 지원)
            ...
            'pin': ['AT+PSWD={}', 'AT+PIN={}', 'AT+PSWD:"{}"', 'AT+PSWD{}', 'AT+PIN{}'],
...
def main():
    ...
    try:
        ...
        # print("\n" + "="*50)
        # print("공장 초기화를 수행하시겠습니까? (y/n): ", end='')
        # if input().lower() == 'y':
        #     hc06.reset_to_factory()
        #     print("초기화 후 전원을 껐다 켜고 다시 실행하세요.")
        #     hc06.close()
        #     return
        
        # === 옵션 2: 사용자 정의 설정 ===
        # 블루투스 이름 변경
        hc06.set_name("MyHC06-014")
        ...
        print("5. 공장 초기화 후 다시 시도 (코드에서 주석 해제)")
